...
阶段顺序由 server/src/stages.js 的 STAGES 定义，server/src/workers/pipeline.js 负责排队、执行、人工门和失败重试：
    ...
    gate_source → script_outline → gate_script → gate_style
      ...
      → avatar_media → avatar_wire → gate_avatar → gate_render → render → done
...
| avatar_media → avatar_wire | HeyGem 口型、章节切片和页面接线 |
| gate_avatar | 逐章数字人验收 |
...
| 更换数字人或重做口型 | avatar_media → avatar_wire | 原文、稿件、PPT、配音、字幕 | HeyGem 口型、切片、接线、最终导出 |
...
数字人不是覆盖层。选择数字人后，演示布局必须永久让出右侧讲师安全区；当前标准预留宽度为 480px（数字人窗口 277×493，2026-07-17 用户拍板）。

avatar_media → avatar_wire 当前负责：合并配音；必要时把源形象转为 H.264；调用 HeyGem；生成 lipsync.mp4；按章节切分预览；应用或校验 AvatarPresenter、Subtitle 组件与 CSS 接线；typecheck 并重新构建预览。
...
## 10.5 效果系统（2026-07-17 现状）

三层结构：**效果本体**在 skill 模板（组件 + CSS，随 scaffold 复制进每个作品）；**使用纪律**在
skills/web-video-presentation/references/CHAPTER-CRAFT.md（镜头与效果件 + 密度档位 + 采用度下限）；
**每作品的实际镜头**是 workspace 的 registry 数据（src/registry/cameraCues.ts），因此对话可调、
确定性可校验（server/src/cameraCheck.js，密度档位 meta.camera.density: restrained/standard/dense，默认 dense）。

- 镜头层 CameraLayer：focus/pan/spotlight/magnify/overview + host/host-full（数字人时刻），
  无变换态测量→取景框夹紧；target 未命中记 window.__vfCameraMisses 原地不动；
  内层 camera-breath 呼吸微推（1.0→1.045/20s）全程自动。
- 数字人 AvatarPresenter：播放+微调同步（禁止逐帧 seek，铁律见 PROJECT-MEMORY），
  host=讲述者时刻（2 倍主位+内容压暗）、host-full=开场全屏（同源模糊填充背景）。
