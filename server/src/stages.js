...
async function wireAvatarWithAgent(job, meta, presDir) {
  ...
  const reserve = positionKey === "right-third" ? 480 : 392;
  const windowWidth = positionKey === "right-third" ? 345 : 277;
  const prompt = [
...
const runners = {
  ...
  async chapter_gen(job) {
    ...
    const avatarReserve = avatar.position === "right-third" ? 480 : 392;
  ...
  async avatar_wire(job) {
    ...
    const positionKey = meta.avatar?.position || "right-third";
    // 尺寸定稿：原 252×448 基础上放大 10%（2026-07-17 用户定稿"再大 10%"）
    // → 277×493；预留区同步加宽保住正文安全区。
    const sizes = positionKey === "right-third"
      ? { reservePx: 480, windowWidthPx: 277, windowHeightPx: 493 }
      : { reservePx: 392, windowWidthPx: 277, windowHeightPx: 493 };
