...
const EFFECTS = new Set(["focus", "pan", "spotlight", "overview"]);
  ...
  for (const [chapterId, steps] of Object.entries(cues)) {
    ...
    (steps ?? []).forEach((cue, stepIndex) => {
      ...
      if (!EFFECTS.has(cue.effect)) {
        findings.push({ rule: "camera-effect", severity: "error", detail: `${where} 未知镜头 "${cue.effect}"（词表：focus/pan/spotlight/overview）` });
        return;
      }
      if (cue.effect !== "overview") {
        moves += 1;
    ...
    if (moves > MOVES_PER_CHAPTER) {
      findings.push({ rule: "camera-budget", severity: "error", detail: `${chapterId} 共 ${moves} 个镜头，超出每章 ${MOVES_PER_CHAPTER} 个预算——镜头为讲解服务，不是炫技` });
    }
  }
