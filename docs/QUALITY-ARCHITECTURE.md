...
任何一层不得按目录字母排序。step 结构变化后必须重做配音、字幕和数字人口型，不能复用旧下游媒体。章节音视频时长误差门槛为 0.2 秒。

**电影字幕契约（2026-07-16 用户定稿，切分器+校验器双重执法）**：①相对整个画面居中（不为数字人小窗偏移——小窗在右下角，10 字单行居中字幕够不到它）；②气口必断（。！？…；：，遇到即切 cue，顿号软断），一句一句展示；③词完整（Intl.Segmenter 中文分词，cue 边界只落词边界，"高/兴"永不分页）；④尾部分隔标点剥离（。，、；：…不显示，？！保留语气且不占字数预算）；⑤正文目标 8 字、硬上限 10 字、单行、时间递增（纯拉丁词/短语豁免上限）。真源：server/templates/gen-subtitle-cues.mjs（与 skill 双副本同步）+ server/src/subtitleCheck.js + stages.js subtitleStyle 预设。

当前已经修复 `audio-segments.json` 顺序被目录排序破坏的问题。

数字人断续感已于 2026-07-16 定位并修复（job-20 实测证据链）：**HeyGem 推理内部本来就按乒乓（正放→倒放→正放）延长素材**——54.5s 素材铺 165.7s 音频，两个折返点抽帧验证无跳切；断续的真因是模板 AvatarPresenter 用"视频永远暂停 + 每帧 rAF seek"驱动口型对齐，而 lipsync.mp4 关键帧间隔 10s，浏览器每次 seek 要从关键帧起解码，每秒 60 次 seek 实际只渲染出几帧。修复：①组件改为"播放 + 微调"（音频播放时视频 play()，|偏差|<0.5s 用 playbackRate 0.9~1.1 追齐，仅换步/换章硬 seek；音频暂停时视频暂停落位）；②avatar_media 下载 lipsync 后重编码 `-g 25`（1s 关键帧），章节切片同参。实测修复后有效帧率 25fps 满帧、0 丢帧、4s 推进 3.98s 与音频锁定。存量 7 个作品的 lipsync 已重编码、9 个作品组件已替换。仍保留的观察项：章节边界静音/波形连续性验证（避免"总时长对了但边界有细缝"）。

字幕对齐的三层排查结论（2026-07-16）：数据层 words.json vs mp3 偏差恒定 -0.04s；预览播放层 rAF 采样实测切换偏差 4-26ms；**成片渲染层曾是漂移源**——帧清单对相同/乱序时间戳垫 8ms 假时长导致视频时间轴累计拉长（配音按真实挂钟摆放，故越播越偏且随负载波动）。已修复为直接丢弃重复帧、时间轴与挂钟严格一致，render-meta.json 记录 droppedFrames/timelineSpanSec。用户报"音画不同步"时先确认看的是预览还是成片、成片渲染于何时。
