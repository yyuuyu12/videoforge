...
① 块级标题 rect 宽=容器宽——可行倍率预检把短标题误判"太宽推不动"整片降级 spotlight。
  三处统一改 **Range 墨迹矩形**（choreographer sig/AI 目标矩形/CameraLayer 守卫）：推近
  保护的是文字墨迹不被切，不是块盒子；② 主标题信号只认 h1 → 泛化"最大号可见文字块"
  （h1-h3 40px 门槛、其余 44px）；**字号会继承**：外层容器与真标题平局且文档序在前，
  平局必须取更深层元素（`>=`）；③ registry 第⑤⑥种形状（def() 工厂/组件+narrations 双
  import 重复计数）→ 静态解析去重 + **新模板 App 暴露 `window.__VF_STRUCTURE` 运行时
  结构真相源**（编排器开页优先读，六种形状后停止源码猜测军备竞赛）；④ 保底强特写
  已收紧只认真 zoom。**用户关切确认：构图均衡规则不是罪魁（prompt+记账而已），无需回退。**

**其他**：`F:\Projects\启动视频工作台.bat`（GBK）一键启动+体检+开页面，模型服务仍走设置页
（token 保护）；生成期增量预览失败改中性提示（树暂时破损是常态、指纹变化自动重试，b97ac1f，
中途曾把 import.meta.env 推坏一版立即修复）；逐屏审计进度回调/取消按钮美化由并行会话完成。
