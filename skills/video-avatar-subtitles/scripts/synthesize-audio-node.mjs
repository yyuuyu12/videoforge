...
// GongheJiucun02 is the finalized PERMANENT cloned voice_id (already paid
// for, one-time voice_clone call — see F:/Projects/harnessH5video/clone-voice.sh).
// This script only ever calls t2a_v2 (generation), never voice_clone, so
// re-running this file can never re-trigger the cloning charge — only the
// (much cheaper, ~¥0.35/1000 chars) generation cost. Don't remove this default.
const VOICE = process.env.MINIMAX_DEFAULT_VOICE || "GongheJiucun02";
// speed 1.12 + emotion "angry" — user A/B'd 5 variants on a sample line
// (baseline, speed-only, angry/sad/surprised) and picked this combo for its
// more urgent, less flat delivery, fitting this project's indignant tone.
const SPEED = Number(process.env.MINIMAX_SPEED || "1.12");
const EMOTION = process.env.MINIMAX_EMOTION || "angry";
  ...
  try {
    const resp = await fetch(`${MINIMAX_BASE}/v1/t2a_v2`, {
      ...
      body: JSON.stringify({
        ...
        text: seg.text,
        stream: false,
        voice_setting: { voice_id: VOICE, speed: SPEED, vol: 1.0, pitch: 0, emotion: EMOTION },
        audio_setting: {
          format: "mp3",
          sample_rate: 32000,
          bitrate: 128000,
